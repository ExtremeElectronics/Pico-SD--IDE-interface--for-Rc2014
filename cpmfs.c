...
static int splitFilename(const char *fullname, int type, char *name, char *ext, int *user) 
{
  ...
  *user=10*(fullname[0]-'0')+(fullname[1]-'0');
  ...
  if (fullname[i]=='.')
  {
    ...
    {
      boo="4:illegal CP/M filename";
      return -1;
    }
    else ext[j]=toupper(fullname[i]);
    if (i==1 && j==0)
    {
      boo="5:illegal CP/M filename";
      return -1;
    }
  }
  return 0;
...
static void alvInit(const struct cpmSuperBlock *d)
{
...
#ifdef CPMFS_DEBUG
    printf("alvInit: allocate directory block %d\n",block);
#endif
  } 
  /*}}}*/
  for (i=0; i<d->maxdir; ++i) /* mark file blocks as used */ /*{{{*/
  {
    ...
    {
#ifdef CPMFS_DEBUG
      printf("alvInit: allocate extent %d\n",i);
#endif
      for (j=0; j<16; ++j)
      {
        ...
        {
#ifdef CPMFS_DEBUG
          printf("alvInit: allocate block %d\n",block);
#endif
          offset=block/INTBITS;
          d->alv[offset]|=(1<<block%INTBITS);
        }
...
#ifdef CPMFS_DEBUG
        printf("allocBlock: allocate data block %d\n",block);
#endif
        drive->alv[i] |= (1<<j);
        return block;
      }
...
static int readBlock(struct cpmSuperBlock *d, int blockno, char *buffer, int start, int end)
{
...
#ifdef CPMFS_DEBUG
  printf("readBlock: read block %d %d-%d\n",blockno,start,end);
#endif
  if (blockno>=d->size)
  {
  ...
  for (counter=0; counter<=end; ++counter)
  {
    ...
    {
#ifdef CPMFS_DEBUG
      printf("readBlock: read sector %d/%d\n",d->skewtab[sect],track);
#endif
...
#ifdef CPMFS_DEBUG
  printf("writeBlock: write block %d %d-%d\n",blockno,start,end);
#endif
  if (end < 0) end=d->blksiz/d->secLength-1;
...
#ifdef CPMFS_DEBUG
  printf("CPMFS: updating time stamps for inode %d (%d)\n",extent,extent&3);
#endif
...
#ifdef CPMFS_DEBUG
  printf("CPMFS: updating ds stamps for inode %d (%d)\n",extent,extent&3);
#endif
...
static int diskdefReadSuper(struct cpmSuperBlock *d, const char *format)
{
  ...
  d->type=0;
  if (f_open(&fp,"diskdefs",FA_READ)!=FR_OK)
  {
    printf("Can't open `diskdefs' \n");
    exit(1);
  }
  ln=1;
    ...
    if (insideDef)
    {
      ...
      else if (argc==2)
      {
        ...
        else if (strcmp(argv[0],"blocksize")==0)
        {
          ...
          if (d->blksiz <= 0)
          {
            printf("invalid blocksize `%s' in line %d\n",argv[1],ln);
            exit(1);
          }
        }
        ...
        else if (strcmp(argv[0],"skewtab")==0)
        {
          ...
          for (pass=0; pass<2; ++pass)
          {
            ...
            for (s=argv[1]; *s; )
            {
              ...
              if (end==s)
              {
                printf("invalid skewtab `%s' at `%s' in line %d\n",argv[1],s,ln);
                exit(1);
              }
              s=end;
              ++sectors;
        ...
        else if (strcmp(argv[0],"offset")==0)  
        {
          ...
          {
            printf("invalid offset value `%s' (%s) in line %d\n",argv[1],strerror(errno),ln);
            exit(1);
          }
          if (endptr==argv[1])
          {
            printf("offset value `%s' is not a number in line %d\n",argv[1],ln);
            exit(1);
          }
          if (*endptr!='\0')
          {
            ...
            switch (toupper(*endptr))
            {
              ...
              case 'T':
                ...
                {
                  printf("offset must be specified after sectrk, tracks and secLength in line %d\n",ln);
                  exit(1);
                }
              ...
              case 'S':
                ...
                break;
              default:
                printf("unknown unit specifier `%c' in line %d\n",*endptr,ln);
                exit(1);
            }
          }
          if (val*multiplier>INT_MAX)
          {
            printf("effective offset is out of range in line %d\n",ln);
            exit(1);
          }
        ...
        else if (strcmp(argv[0],"os")==0)
        {
          ...
          {
            printf( "invalid OS type `%s' in line %d\n",argv[1],ln);
            exit(1);
          }
        }
      ...
      {
        printf("invalid keyword `%s' in line %d\n",argv[0],ln);
        exit(1);
      }
    }
  ...
  if (d->boottrk<0)
  {
    printf( "boottrk parameter invalid or missing from diskdef\n");
    exit(1);
  }
  if (d->secLength<0)
  {
    printf( "secLength parameter invalid or missing from diskdef\n");
    exit(1);
  }
  if (d->sectrk<0)
  {
    printf( "sectrk parameter invalid or missing from diskdef\n");
    exit(1);
  }
  if (d->tracks<0)
  {
    printf( "tracks parameter invalid or missing from diskdef\n");
    exit(1);
  }
  if (d->blksiz<0)
  {
    printf( "blocksize parameter invalid or missing from diskdef\n");
    exit(1);
  }
  return 0;
}
...
int cpmReadSuper(struct cpmSuperBlock *d, struct cpmInode *root, const char *format)
{
  ...
  if (!d->dev.opened) /* create empty directory in core */ 
  {
    memset(d->dir,0xe5,d->maxdir*32);
    printf("Notopened image");
  }
...
#ifdef CPMFS_DEBUG
      printf("getformat: found %d passwords\n",passwords);
#endif
  ...
  for (i=0; i<d->alvSize; ++i)
  {
    ...
    for (j=0; j<INTBITS; ++j)
    {
      if (i*INTBITS+j < d->size)
      {
        ...
        {
#ifdef CPMFS_DEBUG
          printf("cpmStatFS: block %d allocated\n",(i*INTBITS+j));
#endif
          ++buf->f_bused;
        }
        else ++buf->f_bfree;
...
int cpmRead(struct cpmFile *file, char *buf, int count)
{
  ...
  if (file->ino->ino==(ino_t)file->ino->sb->maxdir+1) /* [passwd] */
  {
    ...
    file->pos+=count;
#ifdef CPMFS_DEBUG
    printf("cpmRead passwd: read %d bytes, now at position %ld\n",count,(long)file->pos);
#endif
    return count;
  }
  else if (file->ino->ino==(ino_t)file->ino->sb->maxdir+2) /* [label] */ 
  {
    ...
    file->pos+=count;
#ifdef CPMFS_DEBUG
    printf("cpmRead label: read %d bytes, now at position %ld\n",count,(long)file->pos);
#endif
    return count;
  }
  /*}}}*/
...
#ifdef CPMFS_DEBUG
  printf("cpmRead: read %d bytes, now at position %ld\n",got,(long)file->pos);
#endif
  return got;
}
