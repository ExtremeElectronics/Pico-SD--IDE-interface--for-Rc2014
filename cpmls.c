...
char * UUoldddir(char **dirent, int entries, struct cpmInode *ino)
{
  ...
  int a;
  for(a=0;a<strlen(cbuf);a++) cbuf[a]=0;
  ...
  if (entries)
  {
    ...
    for (l=user=0; user<32; ++user)
    {
      for (i=0; i<entries; ++i)
      {
        ...
        if (dirent[i][0]=='0'+user/10 && dirent[i][1]=='0'+user%10)
        {
          ...
          for (; k<3; ++k)  sprintf(cbuf+strlen(cbuf)," "); //putchar(' ');

          cpmNamei(ino,dirent[i],&file);
          cpmStat(&file,&statbuf);
          sprintf(cbuf+strlen(cbuf)," %5.1ldK",(long) (statbuf.size+buf.f_bsize-1) /
    ...
    sprintf(cbuf+strlen(cbuf),"%5.1d Files occupying %6.1ldK",l,(buf.f_bused*buf.f_bsize)/1024);
    sprintf(cbuf+strlen(cbuf),", %7.1ldK Free.\n",(buf.f_bfree*buf.f_bsize)/1024);
  }
  else sprintf(cbuf+strlen(cbuf),"No files found\n");
