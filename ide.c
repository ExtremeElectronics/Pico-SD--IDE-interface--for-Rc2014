...
//      return 2 + (((t->lba4 & DEVH_HEAD) << 24) | (t->lba3 << 16) | (t->lba2 << 8) | t->lba1);
  ...
  if (t->lba1 == 0) {
    printf( "[Bug: request for sector offset 0].\n");
    t->lba1 = 1;
  }
  cyl = (t->lba3 << 8) | t->lba2;
...
//  if (d->offset == -1 || lseek(d->fd, 512 * (d->offset + d->length - 1), SEEK_SET) == -1) {
  if (d->offset == -1 || f_lseek(&d->fd, 512 * (d->offset + d->length - 1)) != FR_OK) {
    tf->status &= ~ST_DSC;
    tf->status |= ST_ERR;
    tf->error |= ERR_IDNF;
  }
